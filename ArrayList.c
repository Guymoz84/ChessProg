...
ArrayList* arrayListCreate(int maxSize){// init a new list
	if(maxSize<=0){
		Error_Handling(E_COMMAND_LINE,NULL,0);
		return NULL;
	}
	ArrayList* arr =(ArrayList*) malloc(sizeof(ArrayList));
	if(!arr){
		free(arr);
		Error_Handling(E_MEMORY,"arrayListCreate",0);
		exit(0);
		return NULL;
	}
	arr->maxSize=maxSize;
...
ArrayList* arrayListCopy(ArrayList* src){
	...
	if(!arr->elements){
		Error_Handling(E_MEMORY,"arrayListCopy",0);
		arrayListDestroy(arr);
		exit(0);
		return NULL;
	}
...
ArrayList* sortForGetMoves(ArrayList* arraylist){
	qsort(arraylist->elements,arrayListSize(arraylist),sizeof(Move),cmpfunc);
	return arraylist;
}
int cmpfunc(const void * move1, const void * move2){
	if(((Move*)move1)->j>((Move*)move2)->j){
		return -1;
	}
	else if(((Move*)move1)->j==((Move*)move2)->j){
		if(((Move*)move1)->i>((Move*)move2)->i){
			return 1;
		}
			return -1;
	}
